...
Current prereq text target:
  public.courses.prereq_text via --prereq-column prereq_text

Legacy prereq output (not part of the current schema):
  public.course_prereqs(course_id, prereq_text)

Optional offerings output (recommended):
...
Current prereq output:
  python scrape_uwp_catalog.py --out courses.sql --prereq-column prereq_text
...
def parse_offered_terms(offered_text: Optional[str]) -> List[str]:
    if not offered_text:
        return []
    text = offered_text.strip().lower().rstrip(".")
    terms: List[str] = []

    if "occasionally" in text:
        terms.append("OCCASIONALLY")
    if "yearly" in text:
        terms.append("YEARLY")
    if "winterim" in text:
        terms.append("WINTERIM")

    if re.search(r"fall\s*\(odd years\)", text):
        terms.append("FALL_ODD")
    elif re.search(r"fall\s*\(even years\)", text):
        terms.append("FALL_EVEN")
    elif "fall" in text:
        terms.append("FALL")

    if re.search(r"spring\s*\(odd years\)", text):
        terms.append("SPRING_ODD")
    elif re.search(r"spring\s*\(even years\)", text):
        terms.append("SPRING_EVEN")
    elif "spring" in text:
        terms.append("SPRING")
...
def write_courses_sql(
    courses: List[Course],
    out_path: str,
    prereq_column: Optional[str],
    offered_column: Optional[str],
) -> None:
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("-- Generated by scrape_uwp_catalog.py\n")
        f.write("BEGIN;\n\n")
        columns = ["subject", "number", "title", "credits", "description"]
        if prereq_column:
            columns.append(prereq_column)
        if offered_column:
        ...
        for c in courses:
            prereq_sql = "NULL"
            if prereq_column and c.prereq_text:
            ...
            if prereq_column:
                row.append(prereq_sql)
            if offered_column:
                row.append(offered_sql)
            values_lines.append("(" + ",".join(row) + ")")
...
def write_prereqs_sql(courses: List[Course], out_path: str) -> None:
    """
    Legacy helper for older databases that had a separate prereq table.

    Current schema stores raw prerequisite text in public.courses.prereq_text and
    structured prerequisite trees in public.course_req_* tables.
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("-- Generated by scrape_uwp_catalog.py\n")
        f.write("-- Legacy output; current schema uses public.courses.prereq_text\n")
        f.write("-- Requires older public.course_prereqs(course_id, prereq_text)\n")
        f.write("BEGIN;\n\n")
        for c in rows:
            f.write(
...
def write_offerings_sql(courses: List[Course], out_path: str) -> None:
    rows = []
    for c in courses:
        for term_code in parse_offered_terms(c.offered_text):
            rows.append((c.subject, c.number, term_code))
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("-- Generated by scrape_uwp_catalog.py\n")
        f.write("-- Requires public.course_offerings(course_id, term_code)\n")
        f.write("BEGIN;\n\n")
        for subject, number, term_code in rows:
            f.write(
                "INSERT INTO public.course_offerings (course_id, term_code)\n"
                "SELECT id, '{tc}'\n"
                "FROM public.courses\n"
                "WHERE subject = '{sub}' AND number = '{num}'\n"
                "ON CONFLICT (course_id, term_code) DO NOTHING;\n\n"
                .format(
                    tc=sql_escape_literal(term_code),
                    sub=sql_escape_literal(subject),
                    num=sql_escape_literal(number),
                )
            )
        f.write("COMMIT;\n")
...
def main() -> None:
    ...
    ap.add_argument(
        "--out-offerings",
        default=None,
        help="Optional output SQL file for course offerings (recommended).",
    )
    ap.add_argument(
        "--prereq-column",
        default="",
        help="Column name on public.courses to store prereq text. Use empty string to omit (default).",
    )
    ap.add_argument(
        "--offered-column",
        default="",
        help="Column name on public.courses to store offered seasons. Use empty string to omit (default).",
    )
    ...
    prereq_column = (args.prereq_column or "").strip() or None
    ...
    write_courses_sql(courses, args.out, prereq_column, offered_column)
    ...
    if args.out_offerings:
        write_offerings_sql(courses, args.out_offerings)
        print(f"Wrote offerings inserts -> {args.out_offerings}")
